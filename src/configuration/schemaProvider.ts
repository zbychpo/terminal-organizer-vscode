...
export const schemaScheme = 'terminal-organizer-schema';
...
export const schemaUri = vscode.Uri.parse(`${schemaScheme}:${configFileVersions.latest}terminal-organizer.json`);

export function registerSchemaProvider(context: vscode.ExtensionContext) {
  const provider: vscode.TextDocumentContentProvider = {
    async provideTextDocumentContent(uri: vscode.Uri) {
      const fileUri = vscode.Uri.joinPath(context.extensionUri, 'schema', 'terminal-organizer.json');
